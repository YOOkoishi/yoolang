...
def write_history_html(public_dir: Path, runs: list[dict[str, Any]], scope: str = "root") -> str:
    rows_html = "\n".join(
        (
...
<body>
  <header>
    ...
    <div class="muted">保留最近 {MAX_HISTORY} 次默认分支 CI 的性能报告和 QEMU 动态指令数报告。</div>
    <div class="links">
      <a href="{latest_perf}">最新性能报告</a>
      <a href="{latest_instruction}">最新指令数报告</a>
  ...
  <main>
    <section class="table-wrap">
      <table>
        ...
        <tbody>
          {rows_html or '<tr><td colspan="6" class="muted">暂无历史报告。</td></tr>'}
        </tbody>
      </table>
    </section>
  </main>
...
def write_history_pages(public_dir: Path, runs: list[dict[str, Any]]) -> None:
    root_html = write_history_html(public_dir, runs, "root")
    perf_html = write_history_html(public_dir, runs, "perf")
    instruction_html = write_history_html(public_dir, runs, "instruction")
...
def main() -> int:
    ...
    write_history_pages(public_dir, runs)
    return 0
