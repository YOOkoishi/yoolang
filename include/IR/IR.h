...
class ConstIRValue : public BaseIRValue {
  public:
    std::string name;
    enum ConstType { INT, FLOAT } const_type;
...
class AllocIRValue : public BaseIRValue {
  public:
    enum Type { VAR, ARRAY } type;

    std::string var_name;
    std::string data_type;
    int size;

    AllocIRValue(Type t, const std::string &name, const std::string &ty)
        : type(t), var_name(name), data_type(ty) {};
    AllocIRValue(const std::string &name, const std::string &ty) : var_name(name), data_type(ty) {
        type = VAR;
    };
...
class GlobalAllocIRValue : public BaseIRValue {
  public:
    ...
    GlobalAllocIRValue(Type t, std::unique_ptr<BaseIRValue> v, const std::string &d,
                       const std::string &ty)
        : type(t), value(std::move(v)), var_name(d), data_type(ty) {};
...
class GetElemPtrIRValue : public BaseIRValue {
  public:
    ...
    GetElemPtrIRValue(const std::string &r, const std::string &s, std::unique_ptr<BaseIRValue> i)
        : result_name(r), src(s), index(std::move(i)) {};
...
class GetPtrIRValue : public BaseIRValue {
  public:
    ...
    GetPtrIRValue(const std::string &r, const std::string &s, std::unique_ptr<BaseIRValue> i)
        : result_name(r), src(s), index(std::move(i)) {};
...
class LoadIRValue : public BaseIRValue {
  public:
    std::string src;         // 源地址变量名
    std::string result_name; // 结果临时变量

    LoadIRValue() = default;
    LoadIRValue(const std::string &s, const std::string &r) : src(s), result_name(r) {
    }
...
class BranchIRValue : public BaseIRValue {
  public:
    ...
    BranchIRValue(std::unique_ptr<BaseIRValue> cond, const std::string &t_label,
...
class BinaryIRValue : public BaseIRValue {
  public:
    ...
    BinaryIRValue() {};
    BinaryIRValue(Operation op, std::unique_ptr<BaseIRValue> l, std::unique_ptr<BaseIRValue> r,
                  const std::string &result = "")
        : operation(op), left(std::move(l)), right(std::move(r)), result_name(result) {};
